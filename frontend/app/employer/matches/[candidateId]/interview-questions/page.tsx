...
function InterviewQuestionsPageContent() {
  ...
  if (!candidateId || !vacancyId) {
    return (
      <EmptyState
        ...
        description="A candidate and vacancy are required to open interview questions."
        primaryAction={
          <Link
            href="/"
            className="inline-flex min-h-control items-center rounded-button border border-border bg-surface px-4 text-sm font-medium text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus-ring focus-visible:ring-offset-2"
  ...
  return <InterviewQuestionsWorkspace candidateId={candidateId} vacancyId={vacancyId} enabled />;
}
...
export default function InterviewQuestionsPage() {
  return (
    <WorkspaceShell role="employer">
      <Suspense
        fallback={
          <div role="status" aria-label="Loading interview questions" className="space-y-3">
