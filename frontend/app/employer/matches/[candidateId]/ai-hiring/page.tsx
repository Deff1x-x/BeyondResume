...
function AiHiringPageContent() {
  ...
  if (!user) return <EmptyState role="alert" title={isError ? "Session unavailable" : "Sign in required"} description={isError ? "We could not verify your session. Sign in to continue." : "Sign in to view AI Hiring."} primaryAction={<Link href="/login" className={primaryActionClass}>Go to login</Link>} />;
