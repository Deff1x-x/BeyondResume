...
describe("WorkspaceNavigation", () => {
  it("keeps candidate routes and in-page evidence destinations distinct", () => {
    ...
    expect(screen.getAllByRole("link", { name: "GitHub" })[0]).toHaveAttribute("href", "/#github-section");
    expect(screen.getAllByRole("link", { name: "Overview" })[0]).toHaveAttribute("aria-current", "location");
  ...
  it("shows only safe existing employer destinations and keeps mobile navigation operable", () => {
    ...
    expect(screen.getAllByRole("link", { name: "Recommended" })[0]).toHaveAttribute(
      "href",
      "/#top-matches-by-vacancy"
    );
