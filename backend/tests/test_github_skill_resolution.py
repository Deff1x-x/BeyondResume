...
from app.services.github_skill_resolution import resolve_github_skill_candidates
from app.utils.github_skill_extractor import GitHubSkillCandidate


def candidate(target: str, path: str = "package.json") -> GitHubSkillCandidate:
...
def test_duplicate_targets_resolve_once_and_choose_canonical_candidate(monkeypatch) -> None:
    ...
    result = resolve_github_skill_candidates(
        session, (candidate("React", "z/package.json"), candidate("React", "a/package.json"))
    )
