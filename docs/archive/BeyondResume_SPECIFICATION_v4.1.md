...
они MUST агрегироваться в один `context.signals`; отдельные commands или links для каждого signal
запрещены. Aggregation key не включает `rule_id`, потому что identity `EvidenceSkillLink` уже
нормативно определена в §17.4.
...
и не меняют текущую extraction version. После принятия этого текста implementation stages MAY
добавить `rule_id` и downstream DTO/command propagation, но MUST NOT одновременно вводить новую
business logic вне определённого pipeline.
