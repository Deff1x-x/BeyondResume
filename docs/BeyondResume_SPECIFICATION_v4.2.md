...
### 17.4 Skill identity and Evidence mapping

#### Canonical Skill identity

Skill является persisted global ontology entity и имеет стабильный UUID `id`. Skill содержит:

- `id`;
- `canonical_name` — человекочитаемое каноническое имя;
- `normalized_name` — технический unique key;
- `category` — строковая категория ontology;
- `description` — nullable;
- `deprecated` — boolean;
- `ontology_version` — строка;
- `created_at`;
- `updated_at`.

Aliases не хранятся JSON-массивом внутри Skill. `normalized_name` уникален; `canonical_name` не
является identity key.

`normalized_name` и normalized alias формируются детерминированно: input trim-ится, приводится к
Unicode normalization NFKC и lowercase, последовательности whitespace заменяются одним пробелом,
символы `_` и `-` внутри имени заменяются пробелом, затем выполняется повторный trim. Пустой
результат запрещён. Транслитерация не выполняется; значимые символы, включая `+`, `#`, `.`, `/`,
не удаляются. Например: ` Python ` → `python`, `Node-JS`/`Node_JS` → `node js`, `C++` → `c++`,
`C#` → `c#`, `.NET` → `.net`.

#### Skill Alias and name resolution

SkillAlias является отдельной persisted entity и содержит `id`, `skill_id`, `alias`,
`normalized_alias`, `created_at`. `normalized_alias` строится тем же алгоритмом, уникален и
принадлежит ровно одному Skill. Alias не может разрешаться в несколько skills, а
`normalized_alias` не может совпадать с `normalized_name` другого Skill. Canonical
`normalized_name` и aliases образуют одно пространство разрешения имён.

Имя разрешается строго следующим образом: normalize input; lookup `Skill.normalized_name`; если
не найдено — lookup `SkillAlias.normalized_alias`; если найдено — вернуть canonical Skill; иначе
вернуть unknown skill. Fuzzy matching, embeddings и AI для alias resolution не используются.

Для MVP unknown skills не создаются автоматически. Неразрешённое extraction name не создаёт
Evidence-to-skill mapping и существует только внутри extraction result как unresolved skill
candidate. Persisted custom Skill и UnresolvedSkill model не создаются; добавление ontology skills
является отдельным administrative/curation process вне Stage 9.

#### EvidenceSkillLink

EvidenceSkillLink — persisted assertion, что конкретный EvidenceUnit поддерживает конкретный
canonical Skill. Он содержит:
...
EvidenceUnit может иметь несколько EvidenceSkillLink, Skill может иметь несколько
EvidenceSkillLink; link принадлежит одному candidate. `UNIQUE(evidence_unit_id, skill_id,
extraction_method, extraction_version)` определяет identity link. Passport FK, level и aggregated
skill score в link не добавляются.

`extraction_method` принимает только `deterministic`, `ai` или `manual`; если проект использует DB
CHECK constraints для строковых статусов, применяется тот же стиль. `deterministic` означает
фиксированное правило из normalized source data, `ai` — предложение AI extraction process,
`manual` — явное human confirmation/creation. Stage 9.2 не реализует extraction engine.

`extraction_confidence` — decimal от 0.00 до 1.00 включительно и отражает только уверенность в
mapping EvidenceUnit → Skill; он не включает strength_score, ownership, verification или
freshness и не является Passport confidence. Для MVP deterministic и manual используют `1.00`; ai
использует значение AI extractor. Для числового поля применяется DB range constraint в стиле
проекта.

`context` — JSONB object с минимальными source-specific факторами, объясняющими mapping. Он не
является raw source payload и не дублирует README, file tree или полный document; его ключи
определяет source-specific extraction specification, порядок ключей семантически незначим и он не
участвует в identity link. Для будущего GitHub deterministic extraction допустимы, например,
`{"signal":"manifest","path":"package.json","matched_value":"react"}` или
`{"signal":"language","matched_value":"python"}`. Эти примеры не определяют extraction rules.

#### Link idempotency and candidate consistency

Повторное создание link с тем же identity возвращает существующий link без mutation/flush, если
`extraction_confidence` и `context` совпадают; при отличии этих значений существующий link
обновляется in place, второй link не создаётся. Новая `extraction_version` создаёт новый link, а
не изменяет старую версию. Старые link versions не удаляются на Stage 9.2; выбор active version
будет определён Passport rebuild specification.

`candidate_id` дублируется намеренно для candidate-scoped queries. Application service обязан
проверить равенство `EvidenceSkillLink.candidate_id` и `EvidenceUnit.candidate_id`; EvidenceUnit и
link не могут относиться к разным candidate contexts. Skill остаётся global ontology entity без
`candidate_id`. Если cross-table consistency не выражается обычным FK, она обеспечивается service
validation и tests; DB trigger не добавляется.

EvidenceSkillLink хранит только extraction-level explainability: method, version, extraction
confidence и source-specific context. Итоговые skill score, Passport confidence, level, strongest
evidence, source diversity и aggregated explanation принадлежат будущей Passport version
projection.

GitHub EvidenceUnit может поддерживать несколько skills. Будущий GitHub mapping строится только из
persisted normalized snapshot, а не сетевого вызова; deterministic extractor может использовать
languages, manifest paths/content и другие явно разрешённые signals. README и file tree сами по
себе не являются доказательством каждого упомянутого слова. Полный GitHub skill extraction
algorithm определяется отдельным Stage 9.x и не изменяет Stage 7 services.

### 17.5 Deterministic GitHub Skill Extraction

#### Extraction source and consistency

GitHub deterministic extractor работает только с persisted `GitHubRepositorySnapshot`; сетевые
вызовы, GitHubProvider и повторный fetch запрещены. EvidenceUnit уже существует. Extractor получает
конкретные CandidateProfile, GitHubRepository, GitHubRepositorySnapshot и EvidenceUnit и проверяет,
что snapshot принадлежит repository, repository и EvidenceUnit принадлежат candidate,
`EvidenceUnit.source_type` равен `github_repository`, а `source_reference` совпадает с canonical
repository URL. Нарушение любого из этих условий завершает extractor типизированной internal
domain/service error.

MVP разрешает только два signal type: `repository_language` и `dependency_manifest`. Другие signal
types не используются до отдельного нормативного решения.

#### Signals and manifest boundary

`repository_language` создаётся для каждого языка из normalized `languages` persisted snapshot.
Название языка разрешается через Skill resolver; language percentage/bytes MAY присутствовать в
context. Language является достаточным deterministic signal и создаёт максимум один canonical
Skill link. Unknown language не создаёт link и не изменяет ontology. Язык не позволяет выводить
framework/library: Python не означает FastAPI, JavaScript — React, Java — Spring, C# — .NET.

`dependency_manifest` допускается только для `package.json`, `pyproject.toml`, `requirements.txt`,
`pom.xml`, `go.mod`, `Cargo.toml`, `composer.json`, `Gemfile` и файлов с suffix `.csproj`. Lock files
`package-lock.json`, `yarn.lock`, `pnpm-lock.yaml`, `poetry.lock`, `Pipfile.lock`, `Cargo.lock`,
`composer.lock`, `Gemfile.lock` и произвольные файлы не анализируются.

Extractor использует только manifest data, уже присутствующие в persisted normalized snapshot. Он не
читает repository filesystem, raw provider payload или сеть. Текущий GitHub snapshot schema хранит
manifest paths, но не manifest contents и не normalized dependency representation; поэтому
`dependency_manifest` extraction заблокирован до отдельного этапа расширения snapshot contract.
Нельзя молча получать отсутствующие данные из provider или raw payload.

Когда такая normalized manifest data будет определена, разрешаются только следующие keys/data:
...
- pyproject.toml: `project.dependencies`, `project.optional-dependencies`,
  `tool.poetry.dependencies`, `tool.poetry.group.*.dependencies`; Python runtime entry в
  `tool.poetry.dependencies` не является Skill dependency;
- requirements.txt: parsed package names без version specifiers, markers, extras и URLs;
- pom.xml: только dependency `groupId` и `artifactId`, образующие `groupId:artifactId`;
- go.mod: module paths из `require` declarations;
- Cargo.toml: dependency names из `dependencies`, `dev-dependencies`, `build-dependencies`,
  `target.*.dependencies`, `target.*.dev-dependencies`, `target.*.build-dependencies`;
- composer.json: `require`, `require-dev`, без ключа `php` как dependency;
- Gemfile: parsed gem names;
- .csproj: PackageReference `Include`/`Update` values.

Project name/description/plugins/repositories/properties, scripts, package names, keywords,
homepage, repository, author, arbitrary XML element names и другие manifest metadata не являются
signals.

В MVP запрещены mappings из README, repository description, topics, tags/releases, commit messages,
branch/directory/arbitrary file names, source-code identifiers, comments, documentation text,
issues, pull requests, discussions, stars, forks, watchers, contributors, commit count, repository
age, license, CI badges, generated files, vendored dependencies, lock files и transitive
dependencies.

#### Dependency mapping and multiplicity

Dependency name не является Skill identity. `GitHubDeterministicSkillRule` — version-controlled,
статическое, детерминированное правило (не обязательно persisted MVP entity) с `signal_type`,
`manifest_kind`, `normalized_match_value`, `target_skill_name` и optional `ecosystem`. Правила не
используют fuzzy matching, embeddings, AI, package-registry lookup или сеть. Например, dependency
`react` из package.json может иметь rule с target `React`.

`target_skill_name` обязательно проходит существующий `resolve_skill()`. Extractor не создаёт Skill
или SkillAlias. Если target отсутствует в ontology, link не создаётся, unresolved rule target
возвращается в extraction result, а extraction не падает. Dependency input normalizes: trim, Unicode
NFKC, lowercase, уже выполненное parser snapshot removal только version syntax/extras/surrounding
syntax; package scope сохраняется (`@nestjs/core` не становится `core`), Maven coordinate остаётся
`groupId:artifactId`, Go module path — полным, NuGet package name сравнивается case-insensitively.

Один language signal создаёт максимум один link, один dependency rule — ровно один target Skill;
разные signals могут подтверждать один Skill, а один EvidenceUnit — несколько Skills. Несколько
occurrences одной dependency в snapshot не создают несколько links одной identity. Поскольку
EvidenceSkillLink identity не содержит signal identity, все deterministic signals текущей version,
ведущие к одному Skill для одного EvidenceUnit, агрегируются в один link с unique supporting signals.

#### Version, confidence and context

Для extractor `extraction_method` всегда `deterministic`, а единая implementation constant равна
`github-deterministic-v1`. Version меняется при изменении набора links, target Skill,
интерпретации signals, context schema, normalization, mapping rule или при добавлении manifest/signal
type; refactoring без изменения результата version не меняет. Snapshot checksum не является
extraction_version.

Все deterministic links имеют `extraction_confidence = 1.00`: это confidence явного rule mapping, а
не confidence кандидата. Ownership, freshness, verification, repository size, language percentage,
manifest section и devDependency его не снижают.

Context каждого link имеет schema:
...
`signals` — непустой array уникальных objects. Language object содержит `type` =
`repository_language`, `value` normalized language name, `bytes` integer/null, `percentage`
decimal/null. Manifest object содержит `type` = `dependency_manifest`, normalized `manifest` path,
`ecosystem`, normalized dependency `matched_value` и stable `rule_id`. Raw manifest content, version
constraints, full snapshot payload, README и file tree запрещены. Signals сортируются по `type`,
`manifest` или empty string, `ecosystem` или empty string, `matched_value` или `value`, `rule_id` или
empty string; JSON key order незначим.

#### Result, re-extraction and changes

`GitHubSkillExtractionResult` является immutable и содержит `evidence_unit`, `extraction_version`,
детерминированно отсортированные по `Skill.normalized_name` links, `created_count`, `changed_count`,
`unchanged_count`, `removed_count`, `unresolved_signals`, `unresolved_rule_targets`. Unresolved
items не персистятся и UnresolvedSkill model не создаётся.

Extractor формирует полный authoritative set deterministic links текущей version и EvidenceUnit:
создаёт отсутствующие, обновляет изменённые, оставляет совпадающие без mutation, удаляет через
`session.delete()` отсутствующие в desired set links того же EvidenceUnit с method `deterministic` и
текущей version. Manual/AI links, links других versions и других EvidenceUnit не удаляются. Старые
extraction versions сохраняются исторически.

При неизменившемся snapshot и ontology desired set идентичен, DB mutation/flush отсутствуют, links
считаются unchanged. При изменившемся snapshot desired set пересчитывается полностью, новые signals
создают/обновляют links, исчезнувшие удаляются по stale policy. Ontology change не запускает
extractor автоматически; при явном запуске unresolved target может разрешиться, deprecated Skill
разрешается, если resolver его возвращает, automatic deprecated replacement отсутствует. Изменение
ontology может изменить desired set, но само по себе не меняет extraction version.

#### Transaction, concurrency and explainability

Extractor принимает существующий Session, выполняет add/update/delete и flush только при DB
mutations; commit, rollback, retry, nested transaction и network calls запрещены. IntegrityError
пробрасывается, caller владеет transaction. Validation/extraction error до mutation не меняет DB;
после mutation/flush caller решает судьбу transaction. MVP не использует distributed/advisory locks:
correctness опирается на EvidenceSkillLink UNIQUE identity, одну caller-managed transaction и
IntegrityError при race.

Extraction-level explanation строится только из `context.signals`: signal, место его обнаружения,
rule_id и canonical Skill. Natural-language/AI explanation не хранится и не генерируется; нельзя
включать level inference, итоговый confidence, ownership conclusion, ranking или job suitability.

Stage 9.4 определяет отдельный extractor service. Существующий Stage 7 scan orchestration не
изменяется: интеграция `GitHub scan → snapshot → EvidenceUnit → deterministic skill extraction`
выполняется отдельным последующим этапом.

#### Stable deterministic rule identity

Каждое `GitHubDeterministicSkillRule` для signal type `dependency_manifest` MUST содержать явно
заданный `rule_id`. `rule_id` идентифицирует логическое version-controlled mapping rule и не является
UUID, database primary key, позицией правила в registry или производным значением от runtime data.
Production registry MAY быть пустым; требование `rule_id` применяется к каждому объявленному правилу.

Нормативный формат `rule_id`:

```text
^gh_rule\.[a-z0-9_]+(?:\.[a-z0-9_]+)+\.v[1-9][0-9]*$
```

Дополнительные ограничения:

- длина — от 1 до 128 Unicode code points;
- допустимы только lowercase ASCII letters, digits, `_` и `.` в соответствии с regex;
- comparison является case-sensitive, однако uppercase запрещён форматом;
- значение MUST задаваться явно в declaration правила;
- автоматическая генерация через hash, UUID, registry index, object serialization,
  `target_skill_name` или `normalized_match_value` запрещена.

Рекомендуемая структура имени — `gh_rule.<category>.<name>.v<major>`, например
`gh_rule.package.react.v1`. Сегменты `<category>` и `<name>` являются человекочитаемыми, но
`rule_id` в целом остаётся opaque identifier: система MUST NOT извлекать из сегментов business
semantics или восстанавливать поля правила.

В пределах одного effective deterministic rule registry каждый `rule_id` MUST быть уникален.
Registry construction MUST fail fast типизированной configuration/domain error до extraction, если:

- `rule_id` отсутствует или не соответствует нормативному формату;
- два правила имеют одинаковый `rule_id`;
- два правила имеют один lookup identity
  `(signal_type, manifest_kind, ecosystem-or-null, normalized_match_value)`, но различаются по
  `rule_id` или `target_skill_name`;
- один `rule_id` назначен правилам с различающимися contract fields.

После публикации `rule_id` MUST NOT переиспользоваться для другого mapping. Изменение
`signal_type`, `manifest_kind`, `ecosystem`, `normalized_match_value`, `target_skill_name`,
normalization semantics или результата mapping требует нового `rule_id` с увеличенной major version.
Удалённый identifier не может быть назначен новому правилу. Старые identifiers остаются валидными
для исторически сохранённых contexts.

`rule_id` не участвует в выборе правила и не меняет существующий canonical ordering rules. Rule
lookup выполняется только по нормативным match fields. После match значение `rule_id` MUST быть
скопировано без normalization, rewriting или regeneration во все downstream representations
соответствующего manifest signal и в конечный `context.signals[*].rule_id`.

#### Immutable GitHub extraction DTO contracts

DTO deterministic GitHub pipeline являются immutable value objects. Они не владеют transaction,
не выполняют SQL, network calls, provider fetch, filesystem access, ontology mutation, commit,
rollback или flush. Mutable ORM collections и live JSON objects из snapshot не должны передаваться
в DTO без detached immutable/canonical copy.

`GitHubSkillCandidate` представляет один извлечённый signal до ontology resolution. Он MUST
содержать:

- `signal_type`: `repository_language` или `dependency_manifest`;
- `target_skill_name`: имя, которое должно быть передано существующему `resolve_skill()`;
- immutable source signal data, достаточные для построения нормативного context;
- `rule_id`: non-null для `dependency_manifest`, `null` для `repository_language`.

Для `repository_language` source signal data MUST содержать normalized `value`, а также nullable
`bytes` и `percentage`. Manifest fields и `rule_id` для language candidate MUST быть `null` или
отсутствовать в его typed signal variant.

Для `dependency_manifest` source signal data MUST содержать normalized `manifest` path,
`manifest_kind`, `ecosystem`, normalized `matched_value` и exact `rule_id` matched rule. Значения
MUST поступать из persisted normalized snapshot и matched registry rule; candidate MUST NOT
реконструировать, угадывать или вычислять их из raw content.

`ResolvedGitHubSkillCandidate` представляет успешный результат единственного вызова
`resolve_skill()` для одного `GitHubSkillCandidate`. Он MUST содержать:

- исходный `GitHubSkillCandidate` без изменения;
- resolved persisted canonical `Skill` либо его stable `skill_id` вместе с canonical
  `normalized_name`, в соответствии с существующим service boundary.

Resolved candidate MUST NOT содержать выдуманные `candidate_id`, `evidence_unit_id`, confidence,
extraction version или persistence state. Unknown source skill и unresolved rule target не создают
resolved candidate и продолжают обрабатываться через существующие `unresolved_signals` и
`unresolved_rule_targets`.

Resolution pipeline MUST вызывать только существующий `resolve_skill()` и MUST NOT выполнять fuzzy
matching, AI resolution, automatic Skill/SkillAlias creation или дополнительные fallback queries.
Порядок resolved candidates должен оставаться deterministic; `rule_id` manifest candidate проходит
Rule → Candidate → Resolved Candidate без изменений.

#### Signal preservation and aggregation boundary

Deterministic extraction и resolution MUST сохранять все уникальные source signals до command
aggregation. Потеря signal только потому, что другой signal имеет тот же `target_skill_name`, тот же
resolved `skill_id` или тот же canonical Skill, запрещена.

`extract_github_skill_candidates()` MUST возвращать один `GitHubSkillCandidate` для каждого
уникального matched source signal. До ontology resolution разрешён только exact duplicate removal по
canonical typed source-signal identity:

- для `repository_language`:
  `(signal_type, target_skill_name, value, bytes, percentage)`;
- для `dependency_manifest`:
  `(signal_type, target_skill_name, manifest, manifest_kind, ecosystem, matched_value, rule_id)`.

`target_skill_name` отдельно, без source fields, MUST NOT использоваться как deduplication key.
Canonical ordering candidates MUST оставаться deterministic и MUST включать достаточные source
fields для стабильного порядка; `rule_id` MAY использоваться только как final tie-breaker между
различными manifest signals и MUST NOT менять matching semantics.

Resolution layer MUST сохранить один `ResolvedGitHubSkillCandidate` для каждого успешно resolved
source candidate. Несколько candidates, разрешившихся в один canonical Skill, MUST оставаться
отдельными resolved candidates до command aggregation. Resolution implementation MAY memoize
результат `resolve_skill()` по canonical lookup input, чтобы не выполнять одинаковый ontology lookup
повторно, но такая optimization MUST NOT удалять, объединять или переписывать source candidates.

Единственная normative aggregation boundary находится в factory, формирующей
`GitHubEvidenceCommand`. Factory получает полный deterministic sequence resolved candidates и
группирует его по resolved `skill_id` после того, как explicit caller передал согласованные
`candidate_id` и `evidence_unit_id`. Для каждой группы factory:

1. строит canonical typed signal object из каждого исходного candidate;
2. удаляет только полностью равные signal objects по canonical equality;
3. сортирует signals по нормативному порядку `Canonical GitHub evidence context`;
4. создаёт ровно один command для данного `skill_id`.

Новый grouped resolved DTO не требуется: aggregation MUST выполняться поверх sequence
`ResolvedGitHubSkillCandidate`. Command factory MUST NOT читать snapshot повторно, восстанавливать
отброшенные signals, выполнять rule lookup или повторно вызывать `resolve_skill()`.

Следовательно, implementation MUST удалить существующую target-level deduplication до Stage 9.8A,
если она отбрасывает distinct source signals. Это является contract correction, а не изменением
business mapping: количество persisted links по одному Skill остаётся равно одному благодаря
command-level aggregation.

Нормативная последовательность реализации после принятия этого уточнения:

- Stage 9.6C — preserve all unique extraction signals и заменить target-level deduplication на exact
  typed signal deduplication;
- Stage 9.7C — preserve all successfully resolved candidates одного Skill без target-level
  deduplication;
- Stage 9.8A — реализовать immutable command factory и command-level aggregation.

#### GitHub evidence command contract

Перед созданием или reconciliation `EvidenceSkillLink` application/service layer формирует
immutable `GitHubEvidenceCommand`. Command является полной инструкцией для одного desired
`EvidenceSkillLink` и MUST содержать:

- `candidate_id`;
- `evidence_unit_id`;
- `skill_id`;
- `extraction_method` = `deterministic`;
- `extraction_version` = implementation constant текущего extractor;
- `extraction_confidence` = decimal `1.00`;
- detached canonical `context` нормативной GitHub schema.

`candidate_id` и `evidence_unit_id` MUST поступать из уже существующих, переданных caller entities.
Они не могут вычисляться из repository URL, snapshot, Skill, rule, candidate DTO или случайного
identifier. До формирования commands caller MUST проверить существующий invariant
`EvidenceUnit.candidate_id == CandidateProfile.id` и остальные source consistency requirements
этого раздела.

`skill_id` MUST поступать только из resolved canonical Skill. Command factory MUST NOT повторно
вызывать `resolve_skill()` и MUST NOT принимать unresolved target как `skill_id`.

Для одного `(evidence_unit_id, skill_id, deterministic, extraction_version)` формируется ровно один
command. Если несколько deterministic signals текущей extraction version поддерживают один Skill,
они MUST агрегироваться в один `context.signals` на единственной aggregation boundary, определённой
выше; отдельные commands или links для каждого signal запрещены. Aggregation key не включает
`rule_id`, потому что identity `EvidenceSkillLink` уже нормативно определена в §17.4.

Command ordering MUST быть deterministic по canonical `Skill.normalized_name`, затем по stable
`skill_id` как tie-breaker. Повторное построение commands из одинаковых inputs MUST давать
семантически идентичные values и одинаковый порядок без DB mutation.

#### Canonical GitHub evidence context

`GitHubEvidenceCommand.context` MUST иметь ровно следующую top-level schema:
...
`version` MUST совпадать с `extraction_version` command. `signals` MUST быть непустым массивом
уникальных signal objects.

Language signal object содержит ровно:
...
`bytes` и `percentage` MAY быть `null`. `rule_id`, `manifest`, `manifest_kind`, `ecosystem` и
`matched_value` в language signal запрещены.

Manifest signal object содержит ровно:
...
Все manifest fields обязательны и non-empty. `rule_id` MUST точно совпадать с matched rule.
`target_skill_name` не дублируется в signal, поскольку canonical target представлен `skill_id`
link; explanation может получить canonical Skill через relation.

Signal uniqueness определяется canonical equality всех полей соответствующего typed signal object.
Signals сортируются по уже установленному порядку: `type`, `manifest` или empty string, `ecosystem`
или empty string, `matched_value` или `value`, `rule_id` или empty string. При полном равенстве
дубликат удаляется. JSON object key order семантически незначим, но serializer/test fixtures SHOULD
использовать canonical key order примеров выше.

Context MUST NOT содержать raw manifest content, dependency version constraints, full snapshot,
README, file tree, provider payload, repository credentials, scoring fields, ownership conclusion,
freshness conclusion, skill level, Passport confidence, ranking или natural-language/AI
explanation.

#### EvidenceSkillLink builder and persistence boundary

`GitHubEvidenceSkillLinkBuilder`, если выделен как отдельный component, MUST быть pure mapping from
one `GitHubEvidenceCommand` to constructor/update values of `EvidenceSkillLink`. Builder MUST NOT:

- выполнять SQL, lookup или `resolve_skill()`;
- искать CandidateProfile, EvidenceUnit или Skill;
- вычислять или изменять identifiers;
- строить source signals из snapshot;
- вычислять confidence/version;
- выполнять add, delete, flush, commit, rollback или retry.

Builder копирует command fields без semantic transformation. Persistence/reconciliation service
остаётся ответственным за существующие identity, idempotency, update-in-place и stale-link deletion
rules этого раздела. Разделение на command factory, builder и persistence service не меняет
caller-owned transaction boundary.

Для этого contract обязательны tests, подтверждающие:

- validation формата и uniqueness `rule_id`;
- fail-fast для ambiguous duplicate lookup identity;
- immutable Rule, Candidate, Resolved Candidate и Command;
- `rule_id` проходит Rule → Candidate → Resolved Candidate → context без изменений;
- language candidate не получает synthetic `rule_id`;
- IDs command поступают только из explicit caller inputs и resolved Skill;
- несколько signals одного Skill агрегируются в один command/link;
- canonical deduplication и ordering signals;
- одинаковый input создаёт одинаковые commands без mutation;
- builder не выполняет SQL/flush и не изменяет command values;
- пустой production rule registry остаётся допустимым.

Эти дополнения являются уточнением существующего Stage 9 contract. Они не интегрируют extractor в
Stage 7 orchestration, не добавляют API/jobs/models/migrations, не создают production mapping rules
и не меняют текущую extraction version. Version 4.2 дополнительно исправляет прежнюю неоднозначность
aggregation boundary: distinct source signals MUST сохраняться через extraction и resolution, а
объединение по Skill выполняется только при построении command. Implementation stages MAY добавлять
только определённые здесь propagation, signal-preservation и command-aggregation changes и MUST NOT
одновременно вводить новую business logic вне определённого pipeline.

### 17.6 GitHub normalized manifest snapshot

#### Boundary and schema versions

Manifest collection является частью одного GitHub provider fetch: provider получает разрешённые
manifest files вместе с repository metadata, парсит их до persistence и возвращает immutable
normalized DTO без raw GitHub API payload или raw manifest contents. Persisted snapshot хранит
только normalized manifest records. Stage 9.4 extractor использует только persisted normalized
records и не вызывает provider, сеть, filesystem или raw-content parser. Существующая граница
сохраняется: provider → immutable normalized DTO → canonical serializer → persisted JSONB snapshot.

Payload без `schema_version` является historical schema version 1. New snapshots имеют обязательное
integer `schema_version: 2`; неизвестная version вызывает typed unsupported snapshot schema error,
serializer не принимает future versions. Version не зависит от repository contents; изменение
canonical schema требует version increment. Separate SQL column не создаётся.

Version 2 payload имеет ровно следующие keys:
...
Existing fields retain their current semantics. `normalized_manifests` and `manifest_warnings` are
required arrays and may be empty; additional fields and `raw_manifest_contents` are forbidden.

#### Normalized manifests and dependencies

Each normalized manifest is `{path, kind, ecosystem, dependencies}`. Allowed `kind` values are
`package_json`, `pyproject_toml`, `requirements_txt`, `pom_xml`, `go_mod`, `cargo_toml`,
`composer_json`, `gemfile`, `csproj`. Their required ecosystems are respectively `npm`, `python`,
`python`, `maven`, `go`, `cargo`, `composer`, `rubygems`, `nuget`; arbitrary kind/ecosystem strings
are forbidden.

Each dependency is `{name, section, metadata}`: name is non-empty normalized match value, section
is string/null, metadata is JSON object. Raw version constraints, source lines, XML/JSON/TOML
fragments, secrets, credentials, tokens, full dependency URLs/local paths and hashes are forbidden.
Only kind-specific metadata stated below is allowed; additional metadata keys are forbidden.

- package.json: sections `dependencies`, `devDependencies`, `peerDependencies`,
  `optionalDependencies`; name is NFKC/trim/lowercase package name, scoped names remain complete,
  metadata is `{}`. scripts, overrides, resolutions and arbitrary fields are excluded.
- pyproject.toml: sections `project.dependencies`, `project.optional-dependencies.<group>`,
  `tool.poetry.dependencies`, `tool.poetry.group.<group>.dependencies`; metadata is
  `{group: <group|null>}`. Normalize Python distribution names by NFKC/trim/lowercase, collapse
  runs of `-`, `_`, `.` to `-`, then remove extras, version specifiers and markers. The Poetry
  `python` entry is excluded. Direct URL, git and local-path requirements create warnings only.
- requirements.txt: the same Python normalization, section null, metadata `{}`. Only ordinary
  package requirements are accepted; include/constraint directives, editable/VCS/direct URL/local
  path requirements, index options, hashes and unsupported directives warn. Empty lines/comments do
  not warn.
- pom.xml: only project dependency groupId/artifactId; name is lowercase `groupId:artifactId`,
  section `dependencies`, metadata `{group_id, artifact_id}`. Plugins, plugin dependencies,
  dependencyManagement-only entries, repositories, properties and project metadata are excluded.
  Missing fields or unresolved `${...}` warn; Maven property resolution is not performed.
- go.mod: require module paths only; NFKC/trim/lowercase preserves full path, section `require`,
  metadata `{indirect: boolean}`. replace/exclude do not create records.
- Cargo.toml: allowed sections `dependencies`, `dev-dependencies`, `build-dependencies`, and
  `target.<target>.{dependencies,dev-dependencies,build-dependencies}`. NFKC/trim/lowercase does
  not turn `_` into `-`; package rename uses real package name and metadata is
  `{alias: <key|null>, target: <selector|null>}`. Git/path-only unresolved packages warn.
- composer.json: `require`/`require-dev`, NFKC/trim/lowercase, metadata `{}`; exclude `php`,
  `ext-*`, `lib-*`, `composer-plugin-api`, `composer-runtime-api`.
- Gemfile: statically parsed `gem "<name>"` declarations only, NFKC/trim/lowercase, section `gem`,
  metadata `{}`. No Ruby execution, variables, conditionals, loops, eval or dynamic names; unsafe
  declarations warn and git/path/source options are not retained.
- .csproj: PackageReference Include/Update only, NFKC/trim/lowercase name, section
  `PackageReference`, metadata `{}`. ProjectReference, Reference, SDK, TargetFramework and
  arbitrary XML are excluded; missing Include/Update warns.

#### Paths, deduplication and warnings

Manifest path is repository-relative, NFKC-normalized, uses `/`, has no leading slash, NUL, empty,
`.` or `..` segment and cannot leave repository root; path case is preserved. Fixed names are
case-sensitive; `.csproj` suffix is case-insensitive. Symlink targets are not resolved; known
symlink and submodule manifests are ignored with warnings.

Manifest identity is `(path, kind)`: duplicates merge dependencies/warnings before persistence;
conflicting kind for one path is validation error. Dependency identity is `(name, section, canonical
metadata)`; identical records deduplicate while equal names in different sections remain distinct.
Manifests sort by path/kind, dependencies by name/section/canonical JSON metadata, warnings by
path/code/detail.

Each warning is `{path, kind, code, detail}`. Allowed codes: `inaccessible`, `oversized`,
`invalid_utf8`, `malformed`, `unsupported_syntax`, `unsupported_dependency_source`,
`missing_required_field`, `symlink_ignored`, `submodule_ignored`, `dependency_limit_exceeded`,
`manifest_limit_exceeded`. Warnings are persisted, participate in checksum, are not exceptions and
contain no raw content, secret, full URL, stack trace or raw parser message. Detail is stable and
minimal.

#### Partial success, safety and limits

Malformed, inaccessible or oversized individual manifests produce partial normalized snapshots:
successful manifests persist, discovered paths remain in `manifest_paths`, failed files receive
warnings and no normalized record. Fetch fails only for repository identity/metadata failure,
invalid provider result, unsupported snapshot schema, consistency failure or a repository hard limit
that prevents safe snapshot formation. No automatic retry.

MVP limits: 50 discovered manifests, 256 KiB per manifest, 500 normalized dependencies per manifest,
2,000 per repository, 200 warnings per repository, dependency name length 255 and normalized path
length 1,024. Provider applies limits pre-persistence, truncates deterministically by normalized path
and records the appropriate limit warning. XML DTD/external entities/network resolution are
forbidden; parsers never execute code, archive decompression is not used, binary manifests are
...
`normalized_manifests` and `manifest_warnings`; GitHubProvider retains one
`get_repository_snapshot(...)` method. Provider
discovers allowed paths, safely reads content, applies limits and source-specific pure parsers, and
returns normalized records/warnings only. Source Adapter and Dispatcher contracts do not change.

Version 2 serializer includes all fixed keys, recursively creates detached JSON-compatible objects,
uses sorted keys, UTF-8, `ensure_ascii=false`, compact separators and deterministic arrays. Checksum
covers full Version 2 JSON. New manifests/warnings change checksum. A first successful V2 fetch over
a V1 row updates the existing latest row in place with `snapshot_created=false` and
`snapshot_changed=true`; history row is not created.

V1 snapshots remain physically valid without SQL data migration, read as version 1 and have no
normalized manifests/warnings. Dependency extraction returns no available manifest signals for V1;
language extraction still works. V1 is rewritten only by a later successful provider scan; no
automatic backfill occurs.

If only schema/manifests/warnings change checksum, existing Stage 7 persistence and EvidenceUnit
update run normally: EvidenceUnit identity does not change and no new EvidenceUnit is created.
Manifest warnings do not change Stage 9.5 strength_score, ownership_status or verification_status.

#### Transaction, errors and implementation boundary

Manifest collection/parsing is provider fetch before snapshot persistence. Existing external Stage 7
transaction remains fetch → persist snapshot → generate/update EvidenceUnit; services do not commit
or rollback, caller owns transaction, partial manifest success is not transaction failure and no
manifest-specific orchestration flags are added. Skill extraction is not integrated here.

Fatal future errors include unsupported snapshot schema, invalid normalized manifest/dependency,
invalid manifest path, conflicting manifest identity, a hard-limit failure that cannot produce a
safe partial snapshot and provider repository identity failure. Per-file/parser failures are
warnings. IntegrityError is not converted, retried or rolled back internally.

The next implementation stage may add immutable DTOs, pure parsers, fixture contents, provider
normalization, V2 canonicalization, backward-compatible reader and tests; it may extend JSON payload
without SQL migration. It must not also implement extractor, EvidenceSkillLink generation, Passport,
scoring, API or jobs.
...
### 92.5 Full GitHub Scan Orchestration

Полный scan выполняется для одного CandidateProfile и его подключённого GitHubRepository в
обязательном порядке:

1. `fetch_github_repository_snapshot(...)`;
2. `persist_github_repository_snapshot(...)`;
3. `generate_github_repository_evidence(...)`.

Следующий шаг не выполняется при ошибке предыдущего. Все шаги выполняются в одном внешнем
transaction boundary. Orchestration service не делает commit, rollback, retry, не открывает новую
DB session и не управляет её lifecycle; flush может выполняться только вложенными принятыми
services. Commit и rollback принадлежат вызывающему application/API/job layer. Stage 7.6 не
добавляет API или background execution.

Provider передаётся в orchestration service аргументом. Service не создаёт provider, не использует
global provider, provider factory или environment variables и не выбирает demo/live provider.
Snapshot извне не передаётся: он обязательно получается через Stage 7.4A. Orchestrator передаёт
полученный snapshot и соответствующий GitHubRepository в Stage 7.4B, а затем вызывает Stage 7.5;
он не пересчитывает canonical JSON/checksum, не строит evidence projection и не выполняет
дополнительную identity validation.

Result является immutable и содержит repository, persisted snapshot, EvidenceUnit и флаги
`snapshot_created`, `snapshot_changed`, `evidence_created`, `evidence_changed`.

Первый полный scan устанавливает все флаги в true. При повторном scan с идентичным provider
snapshot Stage 7.4B и Stage 7.5 не изменяют записи и не выполняют лишний flush; все флаги false.
При изменённом snapshot существующие persisted snapshot и EvidenceUnit обновляются in place:
`snapshot_created=false`, `snapshot_changed=true`, `evidence_created=false`,
`evidence_changed=true`. Количество GitHubRepositorySnapshot и repository-level EvidenceUnit при
повторных scan не увеличивается.

Типизированные ошибки Stage 7.4A–7.5, включая provider errors и IntegrityError, пробрасываются
без преобразования в HTTP или общий ScanFailedError. При fetch error persistence и evidence
generation не вызываются; при persistence error evidence generation не вызывается. При error на
